...
- 投稿、审核、发布全流程留痕：每一步的状态变化都写入本机审计库，日志轮转或进程重启后依然可查，出问题能直接看到某次投稿卡在哪一环。
- 同一次投稿在下载、投递、审核、发布之间用同一个标识串联，跨环节对账不再靠猜；「依赖尚未就绪」与「真正失败」分开记录，前者不消耗重试次数。
- 每张图片单独留痕：记录尺寸、体积、解码预算、最终处理方式（原图直发 / 安全压缩 / 使用预览图 / 降级为原图文档）及原因，便于区分是资源不足还是素材本身的问题。
- 审核与发布状态可直接查询，新增命令行工具查看某条审核的完整流水；当前运行的版本号与提交号在 `/version` 与 `/health` 中可见，便于确认线上跑的是哪个版本。
- 发布包除原始可执行文件外，同时提供带版本号的 tar.gz / zip 压缩包。
- 日志与审计记录中的令牌、密钥、请求头等敏感信息统一脱敏后再落盘。

## [2.16.1](https://github.com/redtidev1918/TelePost/compare/v2.16.0...v2.16.1) (2026-09-10)

### Fixed

- 图片在解码前按像素模式和尺寸执行资源预算；超预算原图不再进入 Pillow 全量解码，审核使用可选预览，最终发布安全降级为原图文档。
- 图片压缩改为生成临时派生文件，原始素材保持不变，成功、失败和重试路径都会清理临时文件。
- 审核记录先于 Telegram 预览落库；控制消息失败会补偿清理，进程重启会修复未完成记录，避免永久留下无按钮预览。
- `/ready` 仅在数据库迁移、Bot、投稿与审核恢复完成后返回成功，供上游 outbox 冷启动屏障使用。
...
### 修复
- 相册降级为单张发送时，继续保持所选的回复层级，避免图片脱离主贴或回复错位。
- 修复聊天直接投稿时开启剧透遮罩却未生效的问题，单张和相册均正确显示遮罩。
- 修复图片与文档混合投稿后保存的文件类型、文件编号及文件名错配。

### 文档
- 补充相册降级时的回复规则、频道回复与讨论区评论的区别，以及升级验证说明。

## [2.10.42] - 2026-09-05

### 新增（可配置）
- **多图投稿的频道展示方式可调**：超过一组相册（>10 张）时，默认每个相册回复上一个相册（逐级链）；设
  `CHANNEL_ALBUM_REPLY=post` 后改为「后续相册都回复第一条主贴」，整组跟着主贴走，频道里不再嵌套成一串回复。
- **投稿文件数上限可调**：默认单次 50 个文件；设 `API_MAX_FILES`（如 100）可支持超大图集整本投稿（发布侧仍按每组 ≤10 自动分批）。

### 文档
- 配置参考补充两项配置说明，以及已部署实例的调整方法（`fly secrets set` 后自动重启生效）。

## [2.10.41] - 2026-09-05

### 修复
- **大投稿不再超时失败**：审核通过后发布超时提高到 120 秒，20 图等大投稿在网络慢时也能稳定发出。
- **不再重复投稿**：发布结果不确定（网络超时）时，不再自动降级重发相册，避免同一作品被重复投进频道；改为提示先到频道确认是否已发出。

## [2.10.40] - 2026-09-04

### 修复
- 修复关闭 API 审核时自动化投稿误报 502；直接发布与审核投稿均可正常使用。
- 重复投稿复用既有审核记录时，会在审核群提示原审核编号和当前状态，不再静默返回成功；
  API 同时返回 `reused=true`，并补齐旧审核记录缺失的 `target_id`。
- 多 Bot 路由在自动唤醒时会短暂等待子进程就绪，避免首个请求过早返回 502。

### 文档
- 重写安装、配置、命令、API、Webhook、Fly.io、运维、性能、测试与故障排查文档；
  明确推荐的低成本 Fly 拆分拓扑，并移除过期、重复或未经当前版本验证的部署说明。

## [2.10.39] - 2026-09-04

### 修复
- **Fly.io 自动休眠后机器人永久失联**：休眠时不再注销 Telegram Webhook，冷启动时
  也不再丢弃待处理消息；继续使用 auto-stop 省钱时，用户消息和 PixivFlow 投递均可
  正常唤醒 TelePost。
...
### 修复
- **审核「重抓/换一张」超时误报**：`run-once` 串行跑完所有启用 schedule，每个受
  `plan.timeout`（生产 1800s）watchdog 保护，2 个 schedule 最坏约 3600s；TelePost 侧
  子进程超时从 1500s 提到 3900s，避免在下载途中掐断并误报 `timed out after 1500 seconds`。
- **已发布作品软去重**：同一 `idempotency_key` 的 `published` 记录在 7 天内再次投递时
  直接跳过（返回已发布记录），防止 PixivFlow 去重失效把已发布作品重新投进审核群造成
  「审核完了又转发」；`rejected` / 更早的 `published` 仍允许重建审核（换一张/重新考虑）。
...
### 改进（超大图片压缩投稿，不再降级成文件）
- **超过 Telegram 照片上限（约 10 MiB）的本地原图自动压缩**：新增
  `handlers.publish._compress_photo`（Pillow）——尺寸先缩到 4096px，再逐级降
  JPEG 质量（92→60），压到阈值内后**仍按图片发送**，频道里直接看图；
  压缩失败（无法解码/极端情况）才回退为文档发送兜底。
- `reclassify_oversized_photos` 语义从"超限即降级 document"改为"先压缩、
  失败才降级"；压缩后文件名统一 `.jpg`。覆盖 API 投稿与审核暂存两条路径。
- 新增依赖 `Pillow>=10`；新增回归测试 `test_oversized_photo_is_compressed_not_demoted`
  （433 项全过）。
...
### 修复（聊天投稿"发媒体无响应"）
- **底部菜单「📝 开始投稿」按钮不进状态机**：此前 `handle_menu_shortcuts` 直接
  调 `submit()`，只建 DB 会话、不建立 ConversationHandler 内存状态，用户随后
  发送的媒体/文件会掉出状态机、静默无响应。现将该按钮文本作为
  ConversationHandler 的 entry（`Regex 开始投稿$`），点按钮与 `/submit` 命令
  走完全相同的状态机入口。
- **会话外媒体不再石沉大海**：新增 `orphan_media_guard`（group 997）——状态机
  因重启/超时/误操作丢失时，用户直接发媒体会收到明确提示"请先 /submit"，
  而不是没有任何回应；有活跃会话时不打扰流程。
- **埋点**：`validate_state` 无会话行、`end_conversation_with_message` 现在记录
  `user_id` 与触发函数，复现"❌ 会话已过期"时可从日志精确定位来源。
- 430 项测试全过。
