...
- 审核群预览支持回复链：多页图集/多文件进入审核群时，后续每条预览都回复上一条消息，
  在群内视觉上连成一组投稿，而不是零散消息；可通过 `REVIEW_PREVIEW_THREAD=0` 关闭。
- 自动投稿 caption 模板新增 `{{link}}`（Pixiv 作品/小说永久链接）与 `{{spoiler}}`
  （R-18 自动开启 Telegram 剧透）模板变量，配合 PixivFlow 2.10.5 生成「标题 + 简介 +
  标签 + 原链接 + 剧透」的完整投稿说明。
