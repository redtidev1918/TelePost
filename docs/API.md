...
约束：单文件 ≤50MB，单次文件累计 ≤500MB。

### 发布层级（频道与审核群预览一致，见 CONFIGURATION.md）

一次投稿在频道里的消息结构是固定的，全部由统一投递引擎
（`handlers.publish.deliver_items_to_chat`）生成：

1. **主贴**：整条投递的第一条消息，说明文字（caption）只挂在它上面；
2. 顺序固定：图片/视频相册（每 10 个一组）→ GIF/音频（逐条）→ 文档组（每 10 个一组）；
3. 后续每一段都回复前一段，形成一条"主贴 + 回复链"；文档（小说 `.txt`、
   ugoira `.zip` 等）在投稿含媒体时整段作为主贴的回复发出；
4. 超过 10 MiB 的本地图片自动改按文档发送（Telegram 照片单张上限 10 MiB）；
5. 相册发送失败自动降级为逐条发送，整条投稿不失败。
