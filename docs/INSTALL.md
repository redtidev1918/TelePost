...
`./logs` 挂载到容器。生产环境建议固定版本（例如 `2.17.3`），升级前备份 `data/`。
...
Fly.io 使用预构建镜像、持久卷和 Webhook。单 Bot、双 Bot 以及“TelePost 常驻 +
PixivFlow 独立执行端”完整配置见 [FLYIO_DEPLOYMENT.md](FLYIO_DEPLOYMENT.md)。
